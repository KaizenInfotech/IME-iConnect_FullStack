...
public class SmsService : ISmsService
{
    ...
    /// <summary>
    /// Send SMS via MDS Gateway (National/India).
    /// Matches old API: GlobalFuns.SendSMSAdd
    /// GET https://mdssend.in/api.php?username=...&apikey=...&senderid=...&route=OTP&mobile=...&text=...&tempid=...&entityid=...
    /// </summary>
    private async Task<bool> SendNationalSms(string mobileNo, string message)
    {
        ...
        try
        {
            ...
            var requestUrl = $"{url}?username={username}&apikey={apiKey}&senderid={senderId}&route=OTP&mobile={mobileNo}&text={encodedMessage}";
            ...
            if (responseText.Contains("Message Submitted successfully", StringComparison.OrdinalIgnoreCase) ||
                responseText.Contains("sent", StringComparison.OrdinalIgnoreCase) ||
