...
function getItemType(item) {
  if (item.FilePath || item.filePath) return 'File';
  if (item.Link || item.link) return 'Link';
  return 'File';
}

export default function IMelangePage() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await getMelangeItems(String(year));
      const list = res.data?.TBMERListResult?.MERListResult || [];
      const data = list.map(m => ({
        Id: m.MER_ID, Title: m.Title, Link: m.Link, FilePath: m.File_Path,
        PublishDate: m.publish_date, ExpiryDate: m.expiry_date, FinanceYear: m.FinanceYear, TransType: '2',
      }));
      setAllItems(data);
      setItems(data);
    } catch { setError('Failed to load iMelange items'); }
    ...
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '15px' }}>
        <div>
          ...
          <span style={{ fontSize: '14px', fontWeight: 'bold', color: '#333' }}> - iMelange</span>
        </div>
        <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
          ...
          <select value={year} onChange={(e) => setYear(Number(e.target.value))} style={{ height: '32px', border: '1px solid #ccc', borderRadius: '4px', padding: '4px 8px', fontSize: '13px', outline: 'none' }}>
            {years.map(y => <option key={y} value={y}>{y}</option>)}
          </select>
          <button onClick={() => navigate('/imelange/add')} style={{ display: 'flex', alignItems: 'center', gap: '4px', backgroundColor: '#1a297d', color: '#fff', border: 'none', padding: '6px 14px', borderRadius: '4px', fontSize: '13px', cursor: 'pointer' }}>+ Add</button>
      ...
      <div style={{ backgroundColor: '#fff', borderRadius: '8px', overflow: 'hidden', boxShadow: '0 3px 5px 0px rgba(0,0,0,0.06)' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
          ...
          <tbody>
            {items.length === 0 ? (
              <tr><td colSpan={3} style={{ padding: '30px', textAlign: 'center', color: '#999' }}>No iMelange items found.</td></tr>
            ) : (
              items.map((item, idx) => {
                ...
                return (
                  <tr key={item.Id || item.id || idx} style={{ backgroundColor: idx % 2 === 0 ? '#fff' : '#f8f8f8', borderBottom: '1px solid #eee' }}>
                    <td style={{ padding: '10px 16px' }}>
                      <div style={{ fontWeight: '600', color: '#333', fontSize: '13px' }}>{title} | {itemType}</div>
                    ...
                    <td style={{ padding: '10px 8px', textAlign: 'center' }}>
                      <button onClick={() => navigate(`/imelange/${item.Id || item.id}/edit`, { state: { year } })} title="Edit" style={{ width: '28px', height: '28px', borderRadius: '4px', backgroundColor: '#0ead9a', color: '#fff', border: 'none', cursor: 'pointer', display: 'inline-flex', alignItems: 'center', justifyContent: 'center' }}>
      ...
      <ConfirmDialog isOpen={!!deleteTarget} onClose={() => setDeleteTarget(null)} onConfirm={handleDelete} title="Delete" message={`Are you sure you want to delete "${deleteTarget?.Title || deleteTarget?.title}"?`} />
    </div>
  );
}
